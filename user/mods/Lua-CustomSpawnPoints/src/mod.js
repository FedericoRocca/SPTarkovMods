...
class Mod {
    ...
    postDBLoad(container) {
        ...
        this.GetToWork();
        container.resolve("StaticRouterModService").registerStaticRouter(`${this.Name}-/client/locations`, [
            {
                url: "/client/locations",
                action: (url, info, sessionID, output) => {
                    this.GetToWork();
                    return this.container.resolve("LocationCallbacks").getLocationData(url, info, sessionID);
                }
            }
        ], "aki");
    }
    GetToWork() {
        ...
        const databaseServer = this.container.resolve("DatabaseServer").getTables();
        // DEBU9
        ...
        let disableList = (config_json_1.default.DisableFolderList !== undefined && config_json_1.default.DisableFolderList.trim().length > 0) ? config_json_1.default.DisableFolderList.split(",").map(item => item.trim()) : undefined;
                ...
                for (let key = 0; key < newSpawnPointParams.length; key++) {
                    ...
                    for (let categoryIndex = 0; categoryIndex < values.Categories.length; categoryIndex++) {
                        switch (values.Categories[categoryIndex]) {
                            case "Bot":
                                {
                                    if ((config_json_1.default.DisableDefaultSpawnPoints.Bot[map] || false) === true) {
                            ...
                            case "Player":
                                {
                                    if ((config_json_1.default.DisableDefaultSpawnPoints.Player[map] || false) === true) {
                                        values["Infiltration"] = "";
                                        values.Categories.splice(categoryIndex--, 1);
                                    }
                                    else {
                            ...
                            case "Boss":
                                {
                                    if ((config_json_1.default.DisableDefaultSpawnPoints.Boss[map] || false) === true) {
                                        values.Categories.splice(categoryIndex--, 1);
                                    }
                                    else {
                ...
                for (let key in this.defaultSpawnParms[map]) {
                    ...
                    if (currentSpawnPointParms === undefined) {
                        ...
                        if (foundSpawn[0] === false && currentSpawnPointParms.Categories.includes("Bot") === true) {
                            ...
                            currentSpawnPoints.Default.Bot++;
                            if ((config_json_1.default.DisableDefaultSpawnPoints.Boss[map] || false) === false && currentSpawnPointParms.Categories.includes("Boss") === true) {
                        ...
                        if (foundSpawn[0] === false && defaultSpawnPoint.Categories.includes("Bot") === true) {
                            ...
                            currentSpawnPoints.Default.Bot++;
                            if ((config_json_1.default.DisableDefaultSpawnPoints.Boss[map] || false) === false && currentSpawnPointParms.Categories.includes("Boss") === false && defaultSpawnPoint.Categories.includes("Boss") === true) {
